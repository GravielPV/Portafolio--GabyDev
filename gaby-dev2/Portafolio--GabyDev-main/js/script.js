...
function initContactForm() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        const company = document.getElementById('modal-company').value.trim() || 'N/A';
        ...
        const payload = {
            name,
            email,
            _subject: `🚀 Nuevo mensaje de ${name} - Portfolio GabyDev`,
            company,
            subject,
            message
        };
        ...
        const showSuccess = () => {
            button.innerHTML = '<i class="fas fa-check-circle"></i> <span>¡Mensaje Enviado!</span>';
        ...
        const showError = () => {
            button.innerHTML = '<i class="fas fa-exclamation-circle"></i> <span>Error al enviar</span>';
        ...
        try {
            const response = await fetch('https://formsubmit.co/ajax/peraltavasquez100@gmail.com', {
                method: 'POST',
            ...
            // Fallback: guardar en localStorage si AJAX falla
            const messages = JSON.parse(localStorage.getItem('contactMessages') || '[]');
            messages.push({ ...payload, date: new Date().toISOString() });
            localStorage.setItem('contactMessages', JSON.stringify(messages));
            showSuccess();
...
// Service Worker omitido (sw.js no implementado)
...
const screenshots = {
    ...
    picante: {
        ...
        titleEn: 'La Mañana Picante',
        images: [
            'FotoProyecto/PICANTE/2.png',
            'FotoProyecto/PICANTE/1.png',
            'FotoProyecto/PICANTE/3.png',
            'FotoProyecto/PICANTE/4.png',
            'FotoProyecto/PICANTE/5.png',
            'FotoProyecto/PICANTE/6.png'
        ],
        url: 'https://lamananapicante.com/'
    },
    restaurant: {
...
function initContactPageForm() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        const company = (companyEl && companyEl.value.trim()) ? companyEl.value.trim() : 'N/A';
        ...
        const showError = () => {
            button.innerHTML = '<i class="fas fa-exclamation-circle"></i> <span>Error — intenta por WhatsApp</span>';
            button.style.background = 'linear-gradient(135deg, #ef4444, #dc2626)';
            setTimeout(() => {
                button.innerHTML = originalHTML;
                button.style.background = '';
